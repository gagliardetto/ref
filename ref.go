...
func DeduplicateSlice(slice interface{}, fn func(i int) string) interface{} {
	...
	default:
		panic(fmt.Sprintf("Expected a slice/array, but got a %s", reflect.TypeOf(slice).Kind()))
	}
...
func DeduplicateSlice2(slicePtr interface{}, fn func(i int) string) {
	...
	if reflect.TypeOf(slicePtr).Kind() != reflect.Ptr {
		panic(fmt.Sprintf("Expected a pointer to a slice/array, but got a %s", reflect.TypeOf(slicePtr).Kind()))
	}
	...
	default:
		panic(fmt.Sprintf("Expected a pointer to a slice/array, but got a pointer to a %s", ptrElemKind))
	}
}
...
func HasDuplicate(key string, slice interface{}, fn func(i int) string) bool {
	...
	default:
		panic(fmt.Sprintf("Expected a slice or array, but got a %s", reflect.TypeOf(slice).Kind()))
	}
	return false
}
